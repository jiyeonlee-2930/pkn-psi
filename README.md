...
최소 집합은 2,000개로 고정하고 최대 집합만 2,000과 8,000으로 바꿨다.
두 프로토콜 모두 최소 집합 보유자를 수신자로 둔다.
최대 집합이 4배가 되어도 OT 인스턴스는 같다는 것이 이 실험의 요점이다.
...
|---|---|---|---|---|
| 1 | 5080 | 4000 | 1.27 | 1.27 |
| 2 | 10160 | 4000 | 2.54 | 2.54 |
| 10 |50800 | 4000 | 12.70 | 12.70 |
| 50 | 254000 | 4000 | 63.50 | 63.50 |

소수점 둘째 자리까지 일치한다. 정리 3의 경계가 타이트함을 뒷받침한다.
참여자 3명, 최소 집합 2,000개 원소. δ를 키우면 최대 집합만 커지고
최소 집합은 그대로다. 베이스라인은 최대 집합 보유자를, PKN-PSI는
최소 집합 보유자를 수신자로 둔다.

### A3 — 참여자 수 확장

최소 집합 2,000개, δ=5 고정. 두 프로토콜 모두 최소 집합 보유자를 수신자로 둔다.

| n | 대칭키 뻐꾸기 | 대칭키 MPHF | 감소 | 전송량 뻐꾸기 | 전송량 MPHF | 감소 |
|---|---|---|---|---|---|---|
| 3 | 174160 | 92000 | 47.2% | 696980 | 371161 | 46.8% |
| 5 | 428320 | 264000 | 38.4% | 1408960 | 746459 | 47.0% |
| 10 | 1413720 | 1044000 | 26.2% | 3203910 | 1689860 | 47.3% |
| 20 | 4884520 | 4104000 | 16.0% | 6835060 | 3591036 | 47.5% |

전송량 감소율은 참여자 수와 무관하게 47% 부근을 유지한다. 대칭키 연산
감소율은 47.2%에서 16.0%로 줄어드는데, 두 프로토콜이 공유하는 영-공유
항이 (n+1)M으로 커지면서 절감이 일어나는 수신자 측 항을 압도하기 때문이다.

### A4 — 불균형 구간

참여자 3명, 최소 집합 2,000개. 두 프로토콜 모두 최소 집합 보유자를
수신자로 두므로 비닝 구조의 효과만 분리된다.

| δ | 전송량 뻐꾸기 | 전송량 MPHF | 감소 |
|---|---|---|---|
| 1 | 363980 | 253089 | 30.5% |
| 2 | 446480 | 282256 | 36.8% |
| 5 | 696980 | 371473 | 46.7% |
| 10 | 1124480 | 523401 | 53.5% |
| 25 | 2459480 | 997005 | 59.5% |

불균형이 커질수록 전송량 감소가 30.5%에서 59.5%로 확대된다.

## 알려진 한계

**구축 속도.** 순수 파이썬 MPHF 구축은 100k 키에 약 4.5초다. 2²⁴ 원소 실험은 비현실적이므로 확장성 실험 E2의 상한을 2¹⁸로 둔다. 더 큰 규모가 필요하면 `pthash` C++ 바인딩을 연결할 수 있다.

**bits/key.** 본 프로토타입은 파일럿을 압축 없이 저장하여 3.4~2.0 bits/key를 보인다. PHOBIC은 압축 부호화로 2.11 bits/key를 보고하므로, 여기 수치는 상한이다.
